...
def test_history_aware_strategy_is_registered():
    ...
    assert "dividend_low_vol.regime_adaptive" in strategy_ids
...
def test_regime_adaptive_strategy_metadata_is_exposed():
    spec = get_strategy_spec("dividend_low_vol.regime_adaptive")

    assert spec.name == "红利低波状态自适应策略"
    assert spec.benchmark == "512890.XSHG"
    assert spec.raw["rebalance_frequency"] == "weekly"
    assert "trend_breakdown_fast_cut" in spec.raw["risk_controls"]
