...
> 迁移说明：本文档当前归档在 `skyeye/docs/rfc/`，代码主实现已迁至 `skyeye/dividend_scorer/`，旧 `rqalpha/dividend_scorer/` 兼容层已移除。
...
| 模块 | 文件 | 职责 |
|------|------|------|
| 数据获取 | `skyeye/dividend_scorer/data_fetcher.py` | 从 AKShare/Bundle/RQData 获取数据，带本地缓存 |
| 特征引擎 | `skyeye/dividend_scorer/feature_engine.py` | 计算 8 个估值特征 + 2 个置信度修正因子，统一标准化为百分位 |
| 权重计算 | `skyeye/dividend_scorer/weight_calculator.py` | IC_IR 权重计算（正式） + Elastic Net 实验性对照 |
| 分数合成 | `skyeye/dividend_scorer/score_synthesizer.py` | 线性映射 → 加权合成 → 总分输出 |
| 滚动验证 | `skyeye/dividend_scorer/validator.py` | 时间序列滚动验证 + 策略打分器闭环 |
| 主入口 | `skyeye/dividend_scorer/main.py` | 独立运行 + RQAlpha 嵌入两种模式的入口 |
...
RQAlpha 回测执行
        ↓
skyeye/strategy_scorer.py 对回测结果评分
        ↓
策略分数高 → 打分器参数可信
...
│   ├── strategy_scorer.py            # 策略打分器（外层验证用）
│   └── docs/rfc/dividend_scorer_design.md
└── rqalpha/mod_config.yml            # dividend_scorer -> skyeye.mod.rqalpha_mod_dividend_scorer
```

> **为什么统一放在 `skyeye/`？**
>
> 当前仓库将全部自研实现、示例、文档和实验资产统一收敛到 `skyeye/`。这样做的目的不是把代码塞进 `rqalpha/` 内部，而是明确划分上游框架与自研层：`rqalpha/` 保留框架本体，`skyeye/` 承载所有后续自研迭代。项目通过 `pip install -e .` 暴露 `skyeye` 包，mod 侧通过 `skyeye.mod.rqalpha_mod_dividend_scorer` 稳定导入，不再依赖旧兼容壳。
...
```python
# skyeye/mod/rqalpha_mod_dividend_scorer/mod.py
...
```python
# skyeye/dividend_scorer/config.py
