...
用法:
    python skyeye/strategy_scorer.py strategy.py
    python skyeye/strategy_scorer.py strategy.py --cash 200000
    python skyeye/strategy_scorer.py strategy.py --log mid
    python skyeye/strategy_scorer.py --search "平安"
"""
import argparse
import datetime
import math
...
用法:
    python skyeye/strategy_scorer.py <策略文件> [选项]
...
1. 基本用法 - 对策略文件一键打分:
    ...
    python skyeye/strategy_scorer.py my_strategy.py

2. 指定初始资金（默认100万）:

    python skyeye/strategy_scorer.py my_strategy.py --cash 200000

3. 只跑指定窗口（快速调试）:

    python skyeye/strategy_scorer.py my_strategy.py -w 37          # 只跑最新窗口
    python skyeye/strategy_scorer.py my_strategy.py -w 35-37       # 跑最近3个窗口
    python skyeye/strategy_scorer.py my_strategy.py -w 1,10,20,37  # 跑指定窗口

4. 查看更多交易日志:

    python skyeye/strategy_scorer.py my_strategy.py --log mid    # 最近4个窗口
    python skyeye/strategy_scorer.py my_strategy.py --log high   # 全部37个窗口

5. 绘制价格走势 + 买卖点图表:

    python skyeye/strategy_scorer.py my_strategy.py -w 37 --plot        # 单窗口图表
    python skyeye/strategy_scorer.py my_strategy.py -w 35-37 --plot     # 多窗口各一张图

6. 启用额外 mod 并传递配置:

    python skyeye/strategy_scorer.py my_strategy.py --mod dividend_scorer
    python skyeye/strategy_scorer.py my_strategy.py --mod dividend_scorer -mc dividend_scorer.prior_blend 0.7

7. 用自带的示例策略测试:

    python skyeye/strategy_scorer.py rqalpha/examples/demo_strategy.py
    python skyeye/strategy_scorer.py rqalpha/examples/demo_strategy.py --cash 500000 --log mid
...
10. 搜索股票代码:
    ...
    python skyeye/strategy_scorer.py --search "平安"           # 搜索名称包含"平安"的股票
    python skyeye/strategy_scorer.py -s "000001"               # 搜索代码包含 000001 的证券
    python skyeye/strategy_scorer.py -s "510300"               # 搜索 ETF
    python skyeye/strategy_scorer.py -s "^600[0-9]{3}"         # 正则：搜索60开头的上交所股票
    python skyeye/strategy_scorer.py -s "银行"                 # 搜索名称包含"银行"的所有股票
